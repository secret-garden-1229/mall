...
<div class="container">
	...
	<form class="form-horizontal" action="goodUpdate" method="post" enctype="multipart/form-data" onsubmit="return check()">
		...
		<div class="form-group">
			...
			<div class="col-sm-6">
				<input type="text" class="form-control" name="name" value="${good.gname}" required="required">
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-6">
				<select class="form-control" id="select_topic" name="typeId">
					<c:forEach var="type" items="${typeList}">
						<c:if test="${type.id==good.typeId}"><option selected="selected" value="${good.type.id}">${type.type.tname}</option></c:if>
						<c:if test="${type.id!=good.typeId}"><option value="${type.id}">${type.type.tname}</option></c:if>
					</c:forEach>
				</select>
			</div>
		</div>
